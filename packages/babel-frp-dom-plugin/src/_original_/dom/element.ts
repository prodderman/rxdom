...
export function setAttr(
  ...
  if (config.hydratable && name === 'innerHTML') {
    return t.callExpression(registerImportMethod(path, 'innerHTML'), [
      elem,
      value,
    ]);
  }
    ...
    .forEach((attribute) => {
      ...
      if (
        ...
        if (key === 'ref') {
          ...
          results.exprs.push(
            t.expressionStatement(
              setAttr(attribute, elem, key, value.expression, { isSVG, isCE })
            )
          );
        }
      } else {
        ...
        if (value && ChildProperties.has(key)) {
          results.exprs.push(
            t.expressionStatement(
              setAttr(attribute, elem, key, value, { isSVG, isCE })
            )
          );
        } else {
          ...
          results.template += value
            ? `="${escapeBackticks(escapeHTML(value.value, true))}"`
            : '';
        }
      }
    });
